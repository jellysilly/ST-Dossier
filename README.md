...
## Если папка не отзывается

`/dossier-doctor` (или `dossierDoctor()` в консоли браузера) печатает отчёт:
совпал ли слой расширения с экраном, где папка, видима ли она, что на самом
деле окажется под пальцем в её центре и не перекрыл ли её чужой слой. Отчёт
копируется целиком — с ним понятно, дело в расширении или в теме и других
расширениях поверх него.

Строка «слой совпал с экраном» отвечает на самый неочевидный случай. Всё,
что расширение рисует, лежит в одном слое `#dsr-root`, и отсчитывается от
него. Если чужая тема сделает предка этого слоя содержащим блоком нулевой
высоты, слой схлопнется, а вместе с ним уедет за верхнюю кромку экрана и
дело — папка при этом нажимается, но открывать нечего.
